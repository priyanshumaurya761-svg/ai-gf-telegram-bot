...
# 1.5-flash standard and highly stable model hai
...
def gemini_generate(history):
    ...
    # Fixed clean payload without invalid thinking parameters
    payload = {
        "system_instruction": {
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_name = update.effective_user.first_name or "aap"
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_histories[user_id] = []
    await update.message.reply_text("Theek hai 😊 purani memory reset kar di ❤️")
...
async def chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        reply = await asyncio.to_thread(gemini_generate, history)
